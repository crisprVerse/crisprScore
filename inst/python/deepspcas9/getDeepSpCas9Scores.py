...
def Model_Finaltest(sess, TEST_X, filter_size, filter_num, if3d, model, args, load_episode, best_model_path):
    ...
    #OUT.write(format(tuple(TEST_Z.reshape([np.shape(TEST_Z)[0]]))))
    #OUT.write(format(tuple(TEST_Z.reshape([np.shape(TEST_Z)[0]]))))
    #OUT.write("\n")
    np.savetxt(output_file, TEST_Z)
    #OUT.close()
    return
